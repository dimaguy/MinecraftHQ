...
function encrypt(dataStr, keyString){
    ...
    encryptionBuffer = arrayCopyTo(lenBuffer,encryptionBuffer,0,0,4);
    encryptionBuffer = arrayCopyTo(data,encryptionBuffer,0,4,data.length);
...
function decrypt(dataB64, keyString){
    ...
    var expectedLength = bytesToDecimal(plain.slice(0,4));
    return getString(plain.slice(4,expectedLength + 4));
};
//#endregion
...
function fromBase64(str){
    return Uint8Array.from(atob(str.toString()), c => c.charCodeAt(0));
};
